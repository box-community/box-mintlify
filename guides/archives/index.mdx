...
If you plan to only view archives and not modify them, use the `Read all files and folders` instead of the `Read and write all files and folders` scope. Additionally, to be able to assign [storage policy][storage policy] to an archive during its creation, your enterprise must have access to the Box Zones feature and your app must have the Manage users scope.
